...
fn main() {
    let mut nums = BufReader::new(File::open("day20.txt").expect("I know you are existed"))
        ...
        .map(|s| {
            read_int(&mut s.into_bytes().into_iter()) as i64 * 811589153
        }).collect::<Vec<i64>>();
    ...
    for _ in 0..10 {
        for (idx, num) in nums.iter().enumerate() {
            match (*num) % (nums.len() as i64 - 1) {
                ...
                num if num < 0 => {
                    let (cur_pos, next_pos) = (positions[idx], (positions[idx] as i64 + num - 1).rem_euclid(nums.len() as i64) as usize);
                    if cur_pos == next_pos {
                        continue
                    }
                    if next_pos > cur_pos {
    ...
    let target = (0..positions.len())
        .map(|pos| { nums[positions.iter().enumerate()
            .find_map(|(idx, x)| if *x == pos { Some(idx) } else { None }).expect("")] }).collect::<Vec<i64>>();

    println!("control {}", target.iter().sum::<i64>());
